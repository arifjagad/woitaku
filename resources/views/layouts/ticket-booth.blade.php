        ...
        <div class="ticket">
            ...
            <table class="event-table">
                ...
                <tr>
                    <td>Kode Booth</td>
                    <td>:</td>
                    <td>{{ $dataTicket->booth_code }}</td>
                </tr>
                <tr>
                    <td>Harga Tiket</td>
                    <td>:</td>
                    <td>IDR {{ number_format($dataTicket->rental_price, 0, ',', '.') }}</td>
                </tr>
                <tr>
                    <td>Tanggal Aktif Tiket</td>
                    <td>:</td>
                    <td>{{ \Carbon\Carbon::parse($dataTicket->start_date)->translatedFormat('d F Y') }}</td>
                </tr>
                <tr>
                    <td>Status</td>
                    <td>:</td>
            ...
            <p class="text-justify">Tiket ini merupakan bukti pembelian tiket acara. Tiket hanya berlaku pada Hari
                <strong>{{ \Carbon\Carbon::parse($dataTicket->start_date)->translatedFormat('l') }}</strong>,
                <strong>{{ \Carbon\Carbon::parse($dataTicket->start_date)->translatedFormat('d F Y') }}</strong>. Tiket ini tidak akan aktif (tidak dapat
